...
def test_orchestrator_normal_flow():
    ...
    orchestrator.order_manager.update_order_status(
        order_id,
        OrderStatus.COMPLETED,
        reason="test_force_complete",
        force=True,
    )
...
def test_orchestrator_risk_flow():
    ...
    orchestrator.order_manager.update_order_status(
        ...
        reason="test_force_confirmation",
        force=True,
    )
...
def main():
    ...
    try:
        ...
        if faiss_manager.get_document_count() == 0:
            print("提示: FAISS 索引为空，建议先运行 scripts/bootstrap/init_faiss_index.py")
        else:
