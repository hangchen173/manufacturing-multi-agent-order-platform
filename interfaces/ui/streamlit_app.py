...
storage_summary = st.session_state.orchestrator.order_manager.get_storage_summary()
active_order_count = storage_summary.get("active", {}).get("total_count", 0)
archived_order_count = storage_summary.get("archived", {}).get("total_count", 0)
...
with st.sidebar:
    ...
    st.caption(f"活动订单 {active_order_count} / 归档订单 {archived_order_count}")
...
with tab2:
    st.subheader("📊 处理历史")
    st.caption(f"当前展示活动订单，已归档订单数: {archived_order_count}")
    ...
    if orders:
        ...
        for order_id, order in ordered_items:
            last_transition = order.transition_history[-1] if order.transition_history else None
            history_data.append({
                "订单ID": order_id[:8] + "...",
                "状态": order.status.value,
                "创建时间": order.created_at.strftime("%Y-%m-%d %H:%M"),
                "是否需确认": len(order.confirmation_requests) > 0,
                "最近流转": last_transition.to_status.value if last_transition else "-",
                "流转次数": len(order.transition_history),
            })
